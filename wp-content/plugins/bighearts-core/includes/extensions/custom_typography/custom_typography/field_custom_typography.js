...
(function( $ ) {
	...
	redux.field_objects.custom_typography = redux.field_objects.custom_typography || {};

	redux.field_objects.custom_typography.init = function( selector ) {

		selector = $.redux.getSelector( selector, 'custom_typography' );
		$( selector ).each(
			function() {
				var el     = $( this );
				var parent = el;
					...
					function() {
						...
						$( this ).find( '.redux-typography-container' ).each(
							function() {
								...
								var thisID           = $( this ).find( '.redux-typography-family' ).parents( '.redux-container-custom_typography:first' ).data( 'id' );
								...
								redux.field_objects.custom_typography.select( family, true, false, null, true );

								// Init when value is changed.
								$( this ).find( '.redux-typography-family, .redux-typography-family-backup, .redux-typography-style, .redux-typography-style-multi, .redux-typography-subsets, .redux-typography-align' ).on(
									'change',
									function( val ) {
										var getVals;
										var fontName;
										...
										if ( $( this ).hasClass( 'redux-typography-family' ) ) {
											if ( that.val() ) {
												...
												redux.field_objects.custom_typography.select( that, true, false, fontName, true );
											}
										} else {
											val = that.val();
											...
											redux.field_objects.custom_typography.select( $( this ), true, false, null, false );
										}
									}
								);
								...
								$( this ).find( '.redux-typography-size, .redux-typography-height, .redux-typography-word, .redux-typography-letter' ).keyup(
									function() {
										redux.field_objects.custom_typography.select( $( this ).parents( '.redux-container-custom_typography:first' ) );
									}
								);
								...
								reduxTypography.on(
									...
									function() {
										...
										if ( $( this ).hasClass( 'redux-typography-family' ) ) {
											...
											redux.field_objects.custom_typography.select( that, true, false, null, true );
										} else {
											if ( $( this ).hasClass( 'redux-typography-align' ) ||
												$( this ).hasClass( 'redux-typography-subsets' ) ||
												$( this ).hasClass( 'redux-typography-family-backup' ) ||
								...
								redux.field_objects.custom_typography.updates( $( this ) );
	...
	redux.field_objects.custom_typography.updates = function( obj ) {
				...
				$.ajax(
					{
						...
						error: function( response ) {
							...
							redux.field_objects.custom_typography.updates( obj );
						},
	...
	// Return font size.
	redux.field_objects.custom_typography.size = function( obj ) {
		var size = 0;
		var key;
	...
	redux.field_objects.custom_typography.makeBool = function( val ) {
	...
	redux.field_objects.custom_typography.contrastColour = function( hexcolour ) {
		var r;
		var b;
		var g;
		var res;
	...
	redux.field_objects.custom_typography.select = function( selector, skipCheck, destroy, fontName, active ) {
		var mainID;
		var that;
		var family;
		var google;
		...
		mainID = $( selector ).parents( '.redux-container-custom_typography:first' ).data( 'id' );
		if ( undefined === mainID ) {
		...
		style_multi  = that.find( 'select.redux-typography-style-multi').val();

		// Is selected font a google font?
		var style_multi_arr = '';
		if ( true === isSelecting ) {
			google = redux.field_objects.custom_typography.makeBool( selVals['data-google'] );
			that.find( '.redux-typography-google-font' ).val( google );
		} else {
			google = redux.field_objects.custom_typography.makeBool( that.find( '.redux-typography-google-font' ).val() ); // Check if font is a google font.
		}
		...
		if ( active ) {
			...
			if ( ! that.hasClass( 'typography-initialized' ) ) {
				style_multi = that.find( 'select.redux-typography-style-multi' ).data( 'value' );
				...
				if ( true === google ) {
					...
					$.each(
						...
						function( index, variant ) {
							index = null;
							if ( variant.id === style || 1 === redux.field_objects.custom_typography.size( details.variants ) ) {
					...
					that.find( '.redux-typography-style-multi' ).html( html_multi ).select2();
					...
					$.each(
						...
						function( index, subset ) {
							index = null;
							if ( script === subset.id || 1 === redux.field_objects.custom_typography.size( details.subsets ) ) {
								selected = ' selected="selected"';
								script   = subset.id;
								that.find( 'input.typography-subsets' ).val( script );
							} else {
					...
					$.each(
						...
						function( index, variant ) {
							index = null;
							if ( style === variant.id || 1 === redux.field_objects.custom_typography.size( details.variants ) ) {
								selected = ' selected="selected"';
								style    = variant.id;
							} else {
								selected = '';
					...
					if ( that.find( '.redux-typography-style-multi' ) ) {
						$.each(
							default_font_weights,
							function( index, variant ) {
								for (var i = 0; i < style_multi_arr.length; ++i) {
									if ( style_multi_arr[i] === index ) {
										selected_multi = ' selected="selected"';
										break;
									} else {
										selected_multi = "";
								...
								html_multi += '<option value="' + variant.id + '"' + selected_multi + '>' + variant.replace(
									/\+/g, " "
								) + '</option>';
							}
						);
		...
		if ( that.hasClass( 'typography-initialized' ) ) {
			...
			if ( proLoaded ) {
				redux.field_objects.custom_typography.previewShadow( mainID );
			}

			that.find( '.typography-style select2-selection__rendered' ).text( that.find( '.redux-typography-style option:selected' ).text() );

			that.find( '.typography-multi select2-selection__rendered' ).text( that.find( '.redux-typography-style-multi option:selected' ).text() );
